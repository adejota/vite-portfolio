<template>
  <div class="flex flex-col xl:flex-row xl:justify-between p-4 xl:p-16">
    <header
      class="w-full xl:max-w-md xl:mr-8 xl:h-full px-4 pt-4 mb-4 bg-mid-black rounded-xl border border-extra-low-white overflow-hidden relative"
    >
      <div class="flex pb-4">
        <figure>
          <img
        ...
        <div>
          ...
          <div
            class="text-sm text-full-white py-1 px-2 bg-low-black rounded-xl"
          >
        ...
        <button
          @click="setShowHeaderListItem"
          class="absolute top-0 right-0 p-2 xl:hidden"
        >
          <icon :name="setChevronIcon" :class="'w-6 h-6 text-primary'" />
        </button>
      </div>

      <div
        v-if="showHeaderListItem || width >= 1440"
        class="animate__animated animate__fadeIn"
      >
        <div class="flex pt-4 border-b border-t border-extra-low-white">
          <ul>
            <li
              v-for="headerListItem in headerListItems"
              :key="headerListItem.title"
              class="flex mb-4"
            >
              <div
                class="w-8 h-8 mr-2 flex justify-center items-center border border-extra-low-white shadow-lg shadow-full-black rounded-lg"
              >
                <icon
              ...
              <div>
                <p class="text-xs text-mid-white uppercase">
                  {{ headerListItem.title }}
                </p>
                <p class="text-sm text-full-white font-bold">
                  {{ headerListItem.content }}
                </p>
              </div>
            </li>
          </ul>
        ...
        <ul class="flex pb-4">
          <li
            v-for="headerNavListItem in headerNavListItems"
            :key="headerNavListItem.title"
            class="pt-4 mr-2"
          >
    ...
    <section
      class="relative w-full p-4 mb-14 lg:mb-4 xl:mb-0 bg-mid-black rounded-xl border border-extra-low-white overflow-hidden"
    >
      <div
        v-if="selectedNavBarItem === 1"
        class="animate__animated animate__fadeIn"
      >
        <h2 class="text-2xl text-full-white font-bold">Sobre mim</h2>
        <div
          class="mt-2 mb-6 w-12 border-2 rounded-xl border-primary bg-primary"
        ></div>

        <p class="text-mid-white mb-2" v-for="p in aboutMe" :key="p">{{ p }}</p>

        <div class="grid grid-cols-2 lg:grid-cols-2 gap-4 mt-4 mb-8">
          <div
            v-for="aboutListItem in aboutListItems"
            :key="aboutListItem.title"
          ...
          >
            ...
            />
            <h4 class="text-2xl text-full-white font-bold">
              {{ setAboutListItemTitle(aboutListItem) }}
            </h4>
            <p class="text-sm text-mid-white text-center mt-1">
              {{ aboutListItem.content }}
            </p>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 mt-4 mb-8">
          <div
            v-for="doingListItem in doingListItems"
            :key="doingListItem.title"
      ...
      >
        ...
        <ul
          class="text-full-white marker:text-primary list-outside list-disc ml-6 mb-4"
        >
          <li
            v-for="educationListItem in educationListItems"
            :key="educationListItem.title"
            class="pb-4"
          >
            <h4 class="font-bold">{{ educationListItem.title }}</h4>
            <p class="text-primary font-bold text-sm">
              {{ educationListItem.subtitle }}
            </p>
            <p class="text-mid-white">{{ educationListItem.content }}</p>
          </li>
        </ul>

        <h3 class="text-lg text-full-white font-bold mb-4">
        ...
        >
          <li
            v-for="experienceListItem in experienceListItems"
            :key="experienceListItem.title"
            class="pb-4"
          >
            <h4 class="font-bold">{{ experienceListItem.title }}</h4>
            <p class="text-primary font-bold text-sm">
              {{ experienceListItem.subtitle }}
            </p>
        ...
        <div
          class="w-full p-4 mb-8 grid grid-cols-2 gap-2 md:gap-4 md:grid-cols-4 lg:grid-cols-6 border border-extra-low-white shadow-lg shadow-full-black rounded-lg"
        >
          <div
            v-for="skillsListItem in languageSkillsListItems"
            :key="skillsListItem.title"
          ...
          >
            <icon :name="skillsListItem.icon" :class="'w-12 h-12'" />
        ...
        >
          <div
            v-for="skillsListItem in backendSkillsListItems"
            :key="skillsListItem.title"
            class="mr-2 p-2 pt-4 flex flex-col justify-center items-center border border-extra-low-white shadow-lg shadow-full-black rounded-lg"
          >
            <icon :name="skillsListItem.icon" :class="'w-12 h-12'" />
        ...
        >
          <div
            v-for="skillsListItem in frontendSkillsListItems"
            :key="skillsListItem.title"
            class="mr-2 p-2 pt-4 flex flex-col justify-center items-center border border-extra-low-white shadow-lg shadow-full-black rounded-lg"
          >
            <icon :name="skillsListItem.icon" :class="'w-12 h-12'" />
        ...
        >
          <div
            v-for="skillsListItem in testsSkillsListItems"
            :key="skillsListItem.title"
            class="mr-2 p-2 pt-4 flex flex-col justify-center items-center border border-extra-low-white shadow-lg shadow-full-black rounded-lg"
          >
            <icon :name="skillsListItem.icon" :class="'w-12 h-12'" />
        ...
        >
          <div
            v-for="skillsListItem in versioningSkillsListItems"
            :key="skillsListItem.title"
            class="mr-2 p-2 pt-4 flex flex-col justify-center items-center border border-extra-low-white shadow-lg shadow-full-black rounded-lg"
          >
            <icon :name="skillsListItem.icon" :class="'w-12 h-12'" />
      ...
      >
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <div
            v-for="portfolioListItem in portfolioListItems"
            :key="portfolioListItem.name"
          ...
          >
            <div>
              <a :href="portfolioListItem.link" target="_blank">
                <img
                  :src="getImageUrl(portfolioListItem.name)"
                  :alt="`${portfolioListItem.name} project cover`"
                  class="rounded-xl w-full object-cover"
                />
              </a>
              <h3 class="mt-2 text-full-white font-bold">
                {{ portfolioListItem.title }}
              </h3>
              <p class="text-sm text-mid-white mb-2">
                {{ portfolioListItem.content }}
              </p>
            </div>
      ...
      <nav
        class="p-4 bg-mid-black rounded-t-xl overflow-hidden backdrop-blur-lg bg-opacity-20 z-10 fixed bottom-0 left-0 right-0 xl:absolute xl:top-0 xl:right-0 xl:left-1/2 xl:bottom-auto xl:bg-low-black xl:rounded-none xl:rounded-bl-xl xl:border xl:border-extra-low-white overflow-visible"
      >
        <div class="flex justify-evenly">
          <ul class="flex justify-evenly w-10/12">
            <li
              ...
              :class="{
                'font-bold text-primary': selectedNavBarItem === navBarItem.id,
              }"
            >
              <button
                @click="selectedNavBarItem = navBarItem.id"
          ...
          <div class="relative">
            <button @click="showLanguageDialog = !showLanguageDialog">
              <icon name="Language" :class="'w-6 h-6 text-full-white'" />
            </button>
            ...
            >
              <div
                class="py-7 px-5 xl:p-4 border border-extra-low-white bg-mid-black rounded shadow-xl"
              >
                <ul>
                  <li
...
export default {
  ...
  computed: {
    ...
    languageSkillsListItems() {
      return this.skillsListItems.length
        ? this.skillsListItems.filter((i) => i.category === "language")
        : [];
    },

    backendSkillsListItems() {
      return this.skillsListItems.length
        ? this.skillsListItems.filter((i) => i.category === "backend")
        : [];
    },

    frontendSkillsListItems() {
      return this.skillsListItems.length
        ? this.skillsListItems.filter((i) => i.category === "frontend")
        : [];
    },

    testsSkillsListItems() {
      return this.skillsListItems.length
        ? this.skillsListItems.filter((i) => i.category === "tests")
        : [];
    },

    versioningSkillsListItems() {
      return this.skillsListItems.length
        ? this.skillsListItems.filter((i) => i.category === "versioning")
        : [];
    },
  },
